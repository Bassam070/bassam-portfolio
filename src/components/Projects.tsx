...
const Projects = () => {
  ...
  const allProjectsRef = useRef(projects);
  ...
  const filteredProjects = useMemo(() => {
    ...
    return allProjects.filter(
      (project) => normalizeCategory(project.category) === normalizedActive
    );
  ...
  if (filteredProjects.length === 0 && allProjects.length > 0) {
    const normalizedActive = normalizeCategory(activeCategory);
    console.log("[Projects] Empty filter debug", {
      normalizedActive,
      sampleCategory: allProjects[0]?.category,
      normalizedSample: allProjects[0]?.category
        ? normalizeCategory(allProjects[0].category)
        : null,
    });
  }
